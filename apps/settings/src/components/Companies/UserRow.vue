	...
	<Fragment>
		...
		<td class="row__cell row__cell--displayname" :data-test="user.cid">
			<template v-if="user.display">
				<strong
					v-if="!isObfuscated"
					:title="user.display?.length > 20 ? user.display : null"
				>
					{{ user.display }}
				</strong>
			...
			>
				<label class="hidden-visually" :for="'displayName' + uniqueId">
		...
		<td class="row__cell" :class="{ 'row__cell--obfuscated': hasObfuscated }">
			<template
				v-if="
					idState.editing &&
				...
				"
			>
				<label class="hidden-visually" :for="'password' + uniqueId">
		...
		<td class="row__cell row__cell--large">
			<template v-if="idState.editing">
				...
				<NcSelect
					v-model="idState.currentManager"
...
export default {
	...
	computed: {
		...
		isLoadingUser() {
			return (
				this.idState.loading.delete ||
				this.idState.loading.disable ||
				this.idState.loading.wipe
			);
		},
		...
		editedUserQuota: {
			get() {
				...
				if (
					this.settings.defaultQuota !== unlimitedQuota.id &&
					OC.Util.computerFileSize(this.settings.defaultQuota) >= 0
				) {
	...
	methods: {
		wipeUserDevices() {
			...
			OC.dialogs.confirmDestructive(
				t(
					"settings",
					"In case of lost device or exiting the organization, this can remotely wipe the Nextcloud data from all devices associated with {userid}. Only works if the devices are connected to the internet.",
					{ userid }
				),
				...
				(result) => {
					if (result) {
						...
						this.$store
							.dispatch("wipeUserDevices", userid)
							.then(
								() =>
									showSuccess(
										t("settings", "Wiped {userid}'s devices", { userid })
									),
								{ timeout: 2000 }
							)
							.finally(() => {
		...
		async searchUserManager(query) {
			await this.$store
				.dispatch("searchUsers", { offset: 0, limit: 10, search: query })
				.then((response) => {
					const users = response?.data
						? this.filterManagers(Object.values(response?.data.ocs.data.users))
						: [];
					if (users.length > 0) {
		...
		deleteUser() {
			...
			OC.dialogs.confirmDestructive(
				t(
					"settings",
					"Fully delete {userid}'s account including all their personal files, app data, etc.",
					{ userid }
				),
...
.avator-wrapper {
    ...
    background-color: rgb(194, 225, 255) !important;
    color: rgb(25, 118, 204) !important;
	display: flex;
	align-items: center;
